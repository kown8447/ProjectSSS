					...
					success:function(data){
						...
						$.each(data.lists, function(i, elt) {
							text+="<tr style='font-size:x-small; text-align: center;'><td>"+elt.subject_code+"</td><td>"+elt.subject_name+"</td><td>"+elt.registed_seat+"/"+elt.subject_seats+"</td>" +
									"<td>"+elt.subject_credit+"</td><td>"+elt.grade_limit+"</td><td><input type='button' value='강의 정보' style='font-size:8pt;' class='real_info btn btn-xs' id='"+elt.subject_code+"'" +
	...
	$.ajax(
		{
			...
			dataType:"json",
			success:function(data){
				var failText="<table class='table table-hover' style='margin-top:40px'><tr><td colspan='6' style='color:red; font-size:x-small; text-align: center; position:relative;top:expression(this.offsetParent.scrollTop);'>예비 수강신청 실패 과목</td></tr><tr style='text-align:center'><th>과목코드</th><th>과목명</th><th>정원</th><th>학점</th><th>대상학년</th><th>정보</th><th>등록</th><tr/>";
				$('#fail_result').empty();
				$.each(data.failedLists, function(i, elt) {
					...
					"<td>"+elt.subject_credit+"</td><td>"+elt.grade_limit+"</td><td><input type='button' value='강의 정보' class='real_info' id='"+elt.subject_code+"'" +
							"data-target='#real_layerpop' data-toggle='modal'/></td>" +
