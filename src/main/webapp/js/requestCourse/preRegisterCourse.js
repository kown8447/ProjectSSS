				...
				$.each(data.lists, function(i, elt) {
					...
					$.each(elt.period, function(i, obj) {
						...
						if(str[1] == prevDay && str[2] - prev == 1 && obj.period_code.substr(7,1) != 1){
							$('#'+obj.period_code).attr('style','background-color:skyblue');
					...
					success:function(data){
						...
						$.each(data.lists, function(i, elt) {
							text+="<tr><td>"+elt.subject_code+"</td><td>"+elt.subject_name+"</td><td>"+elt.registed_seat+"/"+elt.subject_seats+"</td>" +
									"<td>"+elt.subject_credit+"</td><td><input type='button' value='강의 정보' class='info' id='"+elt.subject_code+"'/></td>" +
	...
	$.ajax(
		{
			...
			success:function(data){
				if(data.result == true){
					...
					beforeSubject(e.currentTarget.id);
				}
			}
		}
	);
