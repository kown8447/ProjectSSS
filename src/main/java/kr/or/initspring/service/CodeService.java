...
public class CodeService {
		...
		if(file != null && file.getSize() > 0){
			try{
				...
				for(int i = 1; i<= last; i++){
					...
					int code_type = Integer.parseInt(row.getCell(1, Row.CREATE_NULL_AS_BLANK).getStringCellValue());
	...
	*/
	...
	//강의실 일괄등록
	@Transactional(rollbackFor = { Exception.class, SQLException.class })
	public boolean classroomExcelList(MultipartHttpServletRequest request, Model model) throws Exception{
					...
					if(file != null && file.getSize() > 0){
						try{
							...
							for(int i = 1; i<= last; i++){
								...
								int scholarship_rcordavg = Integer.parseInt(row.getCell(3, Row.CREATE_NULL_AS_BLANK).getStringCellValue());
					...
					if(file != null && file.getSize() > 0){
						try{
							...
							for(int i = 1; i<= last; i++){
								...
								int department_seat = Integer.parseInt(row.getCell(5, Row.CREATE_NULL_AS_BLANK).getStringCellValue());
								int graduation_credit = Integer.parseInt(row.getCell(6, Row.CREATE_NULL_AS_BLANK).getStringCellValue());
								int double_possible = Integer.parseInt(row.getCell(7, Row.CREATE_NULL_AS_BLANK).getStringCellValue()); 
					...
					if(file != null && file.getSize() > 0){
						try{
							...
							for(int i = 1; i<= last; i++){
								...
								int register_state = Integer.parseInt(row.getCell(4, Row.CREATE_NULL_AS_BLANK).getStringCellValue());
	...
	public int insertAcademicCalendar(Academic_CalendarDTO academic){
		...
		String[] number = office_code.split("_");
