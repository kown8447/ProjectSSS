...
public class RequestCourseService {
	...
	@Value("#{props['correction.AllGrade.startDate']}")
	 ...
	 */
	public String possiblePreRegister(String member_id, Model model) {
		String viewpage = "";
		RequestCourseDAO requestCourseDao = sqlsession.getMapper(RequestCourseDAO.class);
		...
		try {
			...
			if (allgrade_enroll_active == 0) {
				if (enroll_active == 0) {
					viewpage = "requestCourse.notRequestPeriod";
				} else if (enroll_active == 1) {
					viewpage = "requestCourse.preRegisterCourse";
				} else if (enroll_active == 2) {
					String[] timeArray = {};
					if (stStateDto.getGrade() == 1) {
						timeArray = preOneStart.split(" ");
					} else if (stStateDto.getGrade() == 2) {
						timeArray = preTwoStart.split(" ");
					} else if (stStateDto.getGrade() == 3) {
						timeArray = preThreeStart.split(" ");
					} else {
						timeArray = preFourStart.split(" ");
					}
					...
					try {
						startday = sf.parse(year + "-" + timeArray[4] + "-" + timeArray[3] + " " + timeArray[2] + ":"
					...
					long minuteTime = timeterm / 60000;
					int numberhour = (int) (minuteTime / 60);
					int numberMinutes = (int) (minuteTime % 60);
					

					String stringHour = Integer.toString(numberhour);
					String stringMinute = Integer.toString(numberMinutes);
					if (stringHour.length() > 1) {

						model.addAttribute("hourTen", Integer.parseInt(stringHour.split("")[0]));
						model.addAttribute("hour",Integer.parseInt( stringHour.split("")[1]));
					} else {
						model.addAttribute("hour", stringHour);
					}

					if (stringMinute.length() > 1) {
						model.addAttribute("minuteTen", Integer.parseInt(stringMinute.split("")[0]));
						model.addAttribute("minute", Integer.parseInt(stringMinute.split("")[1]));
					} else {
						model.addAttribute("minute", stringMinute);
					}
					model.addAttribute("pageMoveSettingTime", timeterm);
					model.addAttribute("linkAddress", "preRegister.htm");
	 ...
	 */
	public String possibleRealRegister(String member_id, Model model) {
		String viewpage = "";
		RequestCourseDAO requestCourseDao = sqlsession.getMapper(RequestCourseDAO.class);
		StStateDTO stStateDto;
		int enroll_active;
		try {
			...
			if (allgrade_enroll_active == 0) {
				if (enroll_active == 0) {
					...
					String[] timeArray = {};
					if (stStateDto.getGrade() == 1) {
						timeArray = realOneStart.split(" ");
					} else if (stStateDto.getGrade() == 2) {
						timeArray = realTwoStart.split(" ");
					} else if (stStateDto.getGrade() == 3) {
						timeArray = realThreeStart.split(" ");
					} else {
						timeArray = realFourStart.split(" ");
					}
					Calendar cal = Calendar.getInstance();
					SimpleDateFormat sf = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
					Date startday = null;
					...
					model.addAttribute("pageMoveSettingTime", timeterm);
					model.addAttribute("linkAddress", "realRegiser.htm");
					viewpage = "requestCourse.before24Hours";
				}
			} else {
				viewpage = "requestCourse.realRegisterCourse";
	 ...
	 */
	public String possibleCorrectRegister(String member_id, Model model) {
		String viewpage = "";
		RequestCourseDAO requestCourseDao = sqlsession.getMapper(RequestCourseDAO.class);
		int enroll_active;
		try {
			...
			if (enroll_active == 0) {
				...
				viewpage = "requestCourse.correctRegisterCourse";
			} else if (enroll_active == 2) {
				String[] timeArray = afterStart.split(" ");
				Calendar cal = Calendar.getInstance();
				SimpleDateFormat sf = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss");
				Date startday = null;
				int year = cal.get(Calendar.YEAR);
				...
				model.addAttribute("pageMoveSettingTime", timeterm);
				model.addAttribute("linkAddress", "correctRegiser.htm");
				viewpage = "requestCourse.before24Hours";	
			}
	...
	public OpenedLectureDTO getOpSubjectInfoBySubjectCode(String subject_code, String member_id) {
		...
		subject_info.setRequired_choice(
				requestCourseDao.getRequiredChoice(subject_info.getSubject_code(), subject_info.getSubject_type()));
	...
	public List<OpenedLectureDTO> getRealTimetable(String member_id) {
		...
		for (OpenedLectureDTO dto : lists) {
			if (dto.getReserve_check() == 1) {
				...
				dto.setProfessor_name(requestCourseDao.getProfessorNameByPfCode(dto.getProfessor_code()));
				dto.setSubject_filesrc(requestCourseDao.getLecturePlanBySubjectCode(dto.getSubject_code()));
				dto.setRequired_choice(
						requestCourseDao.getRequiredChoice(dto.getSubject_code(), dto.getSubject_type()));
				dto.setRetake_check(
						requestCourseDao.getRetakeCheck(studentDto.getStudent_code(), dto.getSubject_code()));
			}
		}
		return lists;
	}
	...
	public int deleteSubject(String member_id, String subject_code) throws Exception {
		...
		try {
			...
			requestCourseDao.deleteFromEnrollment(studentDto.getStudent_code(), subject_code); // 선택한
																								// 과목
																								// 삭제
			requestCourseDao.minusRegistedSeatBySubjectCode(subject_code); // 수강
																			// 정원
																			// 감소
		} catch (Exception e) {
